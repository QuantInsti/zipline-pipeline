...
from zipline_pipeline.currency import Currency
from zipline_pipeline.data.fx import DEFAULT_FX_RATE
from zipline_pipeline.pipeline.classifiers import Classifier, Latest as LatestClassifier
from zipline_pipeline.pipeline.domain import Domain, GENERIC
from zipline_pipeline.pipeline.factors import Factor, Latest as LatestFactor
from zipline_pipeline.pipeline.filters import Filter, Latest as LatestFilter
from zipline_pipeline.pipeline.sentinels import NotSpecified, sentinel
from zipline_pipeline.pipeline.term import (
    AssetExists,
    LoadableTerm,
    validate_dtype,
)
from zipline_pipeline.utils.formatting import s, plural
from zipline_pipeline.utils.input_validation import (
    coerce_types,
    ensure_dtype,
    expect_types,
)
from zipline_pipeline.utils.numpy_utils import float64_dtype, NoDefaultMissingValue
from zipline_pipeline.utils.preprocess import preprocess
from zipline_pipeline.utils.string_formatting import bulleted_list
...
class BoundColumn(LoadableTerm):
    ...
    latest : zipline_pipeline.pipeline.LoadableTerm
        A :class:`~zipline_pipeline.pipeline.Filter`, :class:`~zipline_pipeline.pipeline.Factor`,
        or :class:`~zipline_pipeline.pipeline.Classifier` computing the most recently
        known value of this column on each date.
        See :class:`zipline_pipeline.pipeline.mixins.LatestMixin` for more details.
    dataset : zipline_pipeline.pipeline.data.DataSet
    ...
    of :class:`~zipline_pipeline.pipeline.data.DataSet`. For example,
    :attr:`~zipline_pipeline.pipeline.data.EquityPricing.close` is an instance of this
    ...
    def fx(self, currency):
        ...
        Parameters
        ----------
        currency : str or zipline_pipeline.currency.Currency
...
class DataSetMeta(type):
    ...
    def specialize(self, domain):
        ...
        -------
        specialized : type
            A new :class:`~zipline_pipeline.pipeline.data.DataSet` subclass with the
    ...
    def qualname(self):
        if self.domain is GENERIC:
            ...
            specialization_key = '<' + self.domain.domain_code + '>'
...
class DataSet(with_metaclass(DataSetMeta, object)):
    ...
    1. A collection of :class:`~zipline_pipeline.pipeline.data.Column` objects that
       describe the queryable attributes of the dataset.

    2. A :class:`~zipline_pipeline.pipeline.domain.Domain` describing the assets and
    ...
    :data:`~zipline_pipeline.pipeline.domain.GENERIC`, which means that they can be used
    ...
    EquityPricing. It is defined as::
        ...
        from zipline_pipeline.pipeline.domain import US_EQUITIES
    ...
    def get_column(cls, name):
        ...
        Returns
        -------
        column : zipline_pipeline.pipeline.data.BoundColumn
...
class DataSetFamilySlice(DataSet):
    ...
    :class:`zipline_pipeline.pipeline.data.dataset.DataSetFamily` objects
    """
...
class DataSetFamily(with_metaclass(DataSetFamilyMeta)):
    ...
    :class:`~zipline_pipeline.pipeline.data.DataSet` objects, each of which has the same
    columns, domain, and ndim.

    :class:`DataSetFamily` objects are defined with one or more
    :class:`~zipline_pipeline.pipeline.data.Column` objects, plus one additional field:
    ``extra_dims``.
    ...
    :meth:`~zipline_pipeline.pipeline.data.DataSetFamily.slice` method.
