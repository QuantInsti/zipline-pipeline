...
from zipline_pipeline.utils.input_validation import expect_types, optional
from zipline_pipeline.utils.memoize import lazyval
from zipline_pipeline.utils.pandas_utils import days_at_time
...
Domain.__qualname__ = "zipline_pipeline.pipeline.domain.Domain"
...
class EquityCalendarDomain(Domain):
    ...
    def __repr__(self):
        return "EquityCalendarDomain({!r}, {!r})".format(
            self.domain_code, self.calendar_name,
        )
...
def infer_domain(terms):
    ...
    Parameters
    ----------
    terms : iterable[zipline_pipeline.pipeline.Term]
