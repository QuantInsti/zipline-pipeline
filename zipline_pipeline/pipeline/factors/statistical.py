...
from zipline_pipeline.pipeline.filters import SingleAsset
from zipline_pipeline.pipeline.mixins import StandardOutputs
from zipline_pipeline.pipeline.sentinels import NotSpecified
from zipline_pipeline.pipeline.term import AssetExists
from zipline_pipeline.utils.input_validation import (
    expect_bounded,
    expect_dtypes,
    expect_types,
)
from zipline_pipeline.utils.math_utils import nanmean
...
class RollingPearson(_RollingCorrelation):
    ...
    Parameters
    ----------
    base_factor : zipline_pipeline.pipeline.Factor
        The factor for which to compute correlations of each of its columns
        with `target`.
    target : zipline_pipeline.pipeline.Term with a numeric dtype
...
class RollingSpearman(_RollingCorrelation):
    ...
    :class:`zipline_pipeline.pipeline.factors.RollingSpearmanOfReturns`
...
class RollingLinearRegression(CustomFactor):
    ...
    Parameters
    ----------
    dependent : zipline_pipeline.pipeline.Factor
        The factor whose columns are the predicted/dependent variable of each
        regression with `independent`.
    independent : zipline_pipeline.pipeline.slice.Slice or zipline_pipeline.pipeline.Factor
    ...
    :class:`zipline_pipeline.pipeline.factors.RollingLinearRegressionOfReturns`
...
class RollingLinearRegressionOfReturns(RollingLinearRegression):
    ...
    :class:`zipline_pipeline.pipeline.CustomFactor`.
...
def vectorized_pearson_r(dependents, independents, allowed_missing, out=None):
    ...
    See Also
    --------
    :class:`zipline_pipeline.pipeline.factors.RollingPearson`
    :class:`zipline_pipeline.pipeline.factors.RollingPearsonOfReturns`
    """
    nan = np.nan
    isnan = np.isnan
