...
from zipline_pipeline.lib.rank import is_missing, grouped_masked_is_maximal
...
from zipline_pipeline.pipeline.term import ComputableTerm, Term
...
class Filter(RestrictedDTypeMixin, ComputableTerm):
    ...
    requested computation. For example, :meth:`zipline_pipeline.pipeline.Factor.top`
    ...
    operators (``<``, ``<=``, ``!=``, ``eq``, ``>``, ``>=``) of
    :class:`~zipline_pipeline.pipeline.Factor`. For example, a natural way to construct
    ...
    20.0::
        ...
        >>> from zipline_pipeline.pipeline.factors import VWAP
    ...
    **either** of its inputs produced True.
    ...
    def if_else(self, if_true, if_false):
        ...
        Parameters
        ----------
        if_true : zipline_pipeline.pipeline.term.ComputableTerm
            Expression whose values should be used at locations where this
            filter outputs True.
        if_false : zipline_pipeline.pipeline.term.ComputableTerm
            Expression whose values should be used at locations where this
            filter outputs False.

        Returns
        -------
        merged : zipline_pipeline.pipeline.term.ComputableTerm
...
class NullFilter(SingleInputMixin, Filter):
    ...
    Parameters
    ----------
    factor : zipline_pipeline.pipeline.Term
...
class PercentileFilter(SingleInputMixin, Filter):
    ...
    Parameters
    ----------
    factor : zipline_pipeline.pipeline.factor.Factor
...
class CustomFilter(PositiveWindowLengthMixin, CustomTermMixin, Filter):
    ...
    :class:`~zipline_pipeline.pipeline.CustomFactor` for more details on
    implementing a custom ``compute`` method.

    See Also
    --------
    zipline_pipeline.pipeline.CustomFactor
    """
    def _validate(self):
        try:
...
class MaximumFilter(Filter, StandardOutputs):
    ...
    def __new__(cls, factor, groupby, mask):
        if groupby is NotSpecified:
            from zipline_pipeline.pipeline.classifiers import Everything
            groupby = Everything()
