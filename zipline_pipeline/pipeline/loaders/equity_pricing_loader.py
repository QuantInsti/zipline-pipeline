...
from zipline_pipeline.data.fx import ExplodingFXRateReader
from zipline_pipeline.lib.adjusted_array import AdjustedArray
from zipline_pipeline.utils.numpy_utils import repeat_first_axis
...
class EquityPricingLoader(implements(PipelineLoader)):
    ...
    Parameters
    ----------
    raw_price_reader : zipline_pipeline.data.session_bars.SessionBarReader
        Reader providing raw prices.
    adjustments_reader : zipline_pipeline.data.adjustments.SQLiteAdjustmentReader
        Reader providing price/volume adjustments.
    fx_reader : zipline_pipeline.data.fx.FXRateReader
    ...
    def _inplace_currency_convert(self, columns, arrays, dates, sids):
        ...
        Parameters
        ----------
        columns : list[zipline_pipeline.pipeline.data.BoundColumn]
    ...
    def _split_column_types(self, columns):
        ...
        Returns
        -------
        ohlcv_columns : list[zipline_pipeline.pipeline.data.BoundColumn]
            Price and volume columns from ``columns``.
        currency_columns : list[zipline_pipeline.pipeline.data.BoundColumn]
