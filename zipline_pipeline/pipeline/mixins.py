...
from zipline_pipeline.lib.labelarray import LabelArray, labelarray_where
from zipline_pipeline.utils.context_tricks import nop_context
from zipline_pipeline.utils.input_validation import expect_dtypes, expect_types
from zipline_pipeline.utils.numpy_utils import bool_dtype
from zipline_pipeline.utils.pandas_utils import nearest_unequal_elements
...
class LatestMixin(SingleInputMixin):
    """
    Common behavior for :attr:`zipline_pipeline.pipeline.data.BoundColumn.latest`.

    Given a :class:`~zipline_pipeline.pipeline.data.DataSet` named ``MyData`` with a
    ...
    resulting expression will be a :class:`~zipline_pipeline.pipeline.CustomFilter` for
    boolean columns, and the resulting object will be a
    :class:`~zipline_pipeline.pipeline.CustomClassifier` for string or integer columns.
    """
    window_length = 1
